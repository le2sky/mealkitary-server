...
class ReservationLineItemTest : AnnotationSpec() {
    ...
    fun `개별 아이템의 수량은 1개 이상이 아니라면 예외를 발생한다`() {
        shouldThrow<IllegalArgumentException> {
            ReservationLineItem.of(ProductId(1L), "부대찌개", Money.of(1000), 0)
    ...
    fun `매핑한 프로덕트와 동등성 검증`() {
        ...
        val item = ReservationLineItem.of(ProductId(1L), "부대찌개", Money.of(1000), 2)
    ...
    fun `개별 아이템의 가격은 수량과 단가의 곱이다`() {
        val item = ReservationLineItem.of(ProductId(1L), "부대찌개", Money.of(10000), 3)
        item.calculateEachItemTotalPrice() shouldBe Money.of(30000)
    }
}
