...
class ReservationTest : AnnotationSpec() {
    ...
    fun `예약 상품이 하나도 존재하지 않는다면 예외를 발생한다`() {
        shouldThrow<IllegalArgumentException> {
            ReservationTestData.defaultReservation().withLineItems().build()
    ...
    fun `예약 하려는 가게가 제공하는 예약 시간이 아니라면 예외를 발생한다`() {
        shouldThrow<IllegalArgumentException> {
            val sut = ReservationTestData.defaultReservation().withReserveAt(
                LocalDateTime.of(
    ...
    fun `예약 하려는 가게가 정상 영업 중이 아니라면 예외를 발생한다`() {
        val invalidShop = ShopTestData.defaultShop().withStatus(ShopStatus.INVALID).build()

        shouldThrow<IllegalStateException> {
            val sut = ReservationTestData.defaultReservation().withShop(invalidShop).build()
            sut.reserve()
    ...
    fun `미결제 상태가 아닌 다른 상태에서 결제를 시도하면 예외를 발생한다`() {
        val base = ReservationTestData.defaultReservation()
    ...
    fun `모든 상품과 가게, 시간이 유효하다면 예약이 가능하다`() {
        val sut = ReservationTestData.defaultReservation().build()
        sut.reserve()
    }
    ...
    fun `예약 생성 요청을 해야 예약의 상태가 미결제로 변경된다`() {
        val sut = spyk(
            objToCopy = ReservationTestData.defaultReservation().build(),
            recordPrivateCalls = true
        )
    ...
    @Test
    fun `총 상품의 가격을 계산한다`() {
        val totalPrice = ReservationTestData.defaultReservation()
    ...
    private fun shouldThrowWhenAcceptBeforeReservationTime(beforeTime: LocalDateTime) {
        shouldThrow<IllegalStateException> {
            ReservationTestData.defaultReservation().withReserveAt(beforeTime).build()
